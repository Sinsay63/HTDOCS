    ...
    function woof_show_btn($autosubmit = 1, $ajax_redraw = 0) {
        ...
        <!--<div class="woof_submit_search_form_container">-->
            ...
            if ($WOOF->is_isset_in_request_data($WOOF->get_swoof_search_slug()) OR $WOOF->is_isset_in_request_data('min_price') OR ( class_exists("WOOF_EXT_TURBO_MODE") AND isset($WOOF->settings["woof_turbo_mode"]["enable"]) AND $WOOF->settings["woof_turbo_mode"]["enable"] )): global $woof_link;
                ...
                <?php
                $woof_reset_btn_txt = __('Réinitialiser', 'woocommerce-products-filter');
                $woof_reset_btn_txt = WOOF_HELPER::wpml_translate(null, $woof_reset_btn_txt);
                ?>

                <?php if ($woof_reset_btn_txt != 'none'): ?>
                    <!--<a href="https://transfosmary.mycoqpit.fr/rechercher-un-produit/"> <button  class="button woof_reset_search_form" data-link=""><?php // echo $woof_reset_btn_txt ?></button></a>-->
                <?php endif; ?>
            <?php endif; ?>

            <?php if (!$autosubmit OR $ajax_redraw): ?>
                <?php
                $woof_filter_btn_txt = __('Filtrer', 'woocommerce-products-filter');
...
<?php endif; ?>
<script>
    const divsearch = document.querySelector('.woof_show_text_search_container');
    let ahref = document.createElement('a');
    ahref.setAttribute('href', 'https://transfosmary.mycoqpit.fr/rechercher-un-produit/');
    let resetButton = document.createElement('button');
    resetButton.innerText= 'Réinitialiser la recherche';
    resetButton.classList.add('button');
    ahref.appendChild(resetButton);
    divsearch.appendChild(ahref);
</script>